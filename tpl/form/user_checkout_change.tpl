        ...
        [{block name="user_checkout_change_form"}]
            ...
            [{block name="user_checkout_change_next_step_top"}]
                <div class="well well-sm cart-buttons">
                    <a href="[{oxgetseourl ident=$oViewConf->getBasketLink()}]" class="btn btn-outline-dark float-sm-left prevStep submitButton largeButton" id="userBackStepTop"><i class="fas fa-caret-left"></i> [{oxmultilang ident="PREVIOUS_STEP"}]</a>
                    <button id="userNextStepTop" class="btn btn-primary float-sm-right submitButton largeButton nextStep" name="userform" type="submit">[{oxmultilang ident="CONTINUE_TO_NEXT_STEP"}] <i class="fas fa-caret-right"></i></button>
                        ...
                        <div class="panel-heading">
                            [{block name="user_checkout_billing_head"}]
                                <h3 class="panel-title">
                                    ...
                                    <button id="userChangeAddress" class="btn btn-xs btn-warning float-sm-right submitButton largeButton" name="changeBillAddress" type="submit" title="[{oxmultilang ident="CHANGE"}]">
                            ...
                            [{block name="user_checkout_shipping_head"}]
                                <h3 class="panel-title">
                                    ...
                                    [{if $oView->showShipAddress() and $oxcmp_user->getSelectedAddress()}]
                                        <button id="userChangeShippingAddress" class="btn btn-xs btn-warning float-sm-right submitButton largeButton" name="changeShippingAddress" type="submit" [{if !$oView->showShipAddress() and $oxcmp_user->getSelectedAddress()}] style="display: none;" [{/if}] title="[{oxmultilang ident="CHANGE"}]">
                                            <i class="fas fa-pencil"></i>
                                        </button>
                                    [{/if}]
                                </h3>
            ...
            [{block name="user_checkout_change_next_step_bottom"}]
                <div class="well well-sm cart-buttons">
                    <a href="[{oxgetseourl ident=$oViewConf->getBasketLink()}]" class="btn btn-outline-dark float-sm-left prevStep submitButton largeButton" id="userBackStepBottom"><i class="fas fa-caret-left"></i> [{oxmultilang ident="PREVIOUS_STEP"}]</a>
                    <button id="userNextStepBottom" class="btn btn-primary float-sm-right submitButton largeButton nextStep" name="userform" type="submit">[{oxmultilang ident="CONTINUE_TO_NEXT_STEP"}] <i class="fas fa-caret-right"></i></button>
                    <div class="clearfix"></div>
                </div>
            [{/block}]
        [{/block}]
