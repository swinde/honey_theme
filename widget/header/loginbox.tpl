...
[{if !$oxcmp_user->oxuser__oxpassword->value}]
    <form class="form" id="login" name="login" action="[{$oViewConf->getSslSelfLink()}]" method="post">
        <div id="loginBox" class="dropdown" [{if $bIsError}]style="display: block;"[{/if}]>
            ...
            <div class="form-group">
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <a class="forgotPasswordOpener btn btn-outline-dark" href="[{oxgetseourl ident=$oViewConf->getSslSelfLink()|cat:"cl=forgotpwd"}]" title="[{oxmultilang ident="FORGOT_PASSWORD"}]">?</a>
                    </span>
                </div>
            </div>
